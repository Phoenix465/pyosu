...
class DifficultySettings(ABC):
    def __init__(self,
        ...
        self.slider_tick_rate = float(slider_tick_rate)


class EventsSettings(ABC):
    def __init__(self,
                 background: dict,
                 video: dict,
                 breakList: dict):
        self.backgroundPath = background["Path"] if background != {} else None
        self.backgroundOffset = (int(background["xOffset"]), int(background["yOffset"])) if background != {} else None

        self.videoPath = video["filename"] if video != {} else None
        self.videoStartTime = int(video["startTime"]) if video != {} else None
        self.videoOffset = (int(video["xOffset"]), int(video["yOffset"])) if video != {} else None

        self.breaks = [BreakObject(**breakData) for breakData in breakList]


class BreakObject(ABC):
    def __init__(self,
                 startTime,
                 endTime):
...
class TimingObject(ABC):
    ...
    def __repr__(self):
        return f"TimingObject(time={self.time},beat_length={self.beat_length},meter={self.meter})"
...
class Difficulty(DifficultySettings):
    def __init__(self, **kwargs):
        if not kwargs.get('approach_rate'):
            kwargs['approach_rate'] = kwargs['overall_difficulty']

        super().__init__(**kwargs)


class Events(EventsSettings):
    def __init__(self, **kwargs):
        if not kwargs.get('video'):
            kwargs['video'] = {}

        if not kwargs.get('breakList'):
            kwargs['breakList'] = []

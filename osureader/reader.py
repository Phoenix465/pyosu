...
class BeatmapParser:
    ...
    def __parse_line(self, line: str, sector: str, beatmap_dict: dict):
        ...
        if sector == "TimingPoints":
            ...
            if line.startswith("//") or "Events" in line:
                return
            ...
            if line[0] == "0":
                ...
                value = {
                    "Path": lineSplit[1],
                    "xOffset": lineSplit[3] if len(lineSplit) > 3 else 0,
                    "yOffset": lineSplit[4] if len(lineSplit) > 4 else 0,
                }

                beatmap_dict[sector][eventName] = value

            elif line[0] == "1" or line[0] == "V":
                eventName = "video"
                lineSplit = line.split(",")

                value = {
                    "startTime": lineSplit[1],
            ...
            elif line[0] == "2" or line[0] == "B":
                eventName = "breakList"
                lineSplit = line.split(",")

                if eventName not in beatmap_dict[sector]:
                    beatmap_dict[sector][eventName] = []

                beatmap_dict[sector][eventName].append({
                    "startTime": lineSplit[1],
                    "endTime": lineSplit[2]
                })
